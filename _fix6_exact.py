#!/usr/bin/env python3
"""Apply Fix 6 - KeyPool mark_rpm/mark_daily in 429 handler - EXACT MATCH"""
...
# Fix 6A: Daily quota handler - exact match from current file
...
if old_daily in code:
    ...
    # Try debugging
    print("Looking for daily quota pattern...")
    if "daily quota exhausted" in code:
        print("  -> Found 'daily quota exhausted' substring")
# Fix 6B: Rate limit handler - exact match  
old_rpm = """                        else:
...
if old_rpm in code:
    ...
    if "_GEMINI_RATE_LIMITED = True" in code:
        print("  -> Found '_GEMINI_RATE_LIMITED = True' substring")
with open(backend_file, 'w', encoding='utf-8') as f:
    f.write(code)
print("\n=== Compilation check: backend.py ===")
import os
result = os.system(f"python3 -m py_compile {backend_file}")

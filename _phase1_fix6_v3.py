#!/usr/bin/env python3
"""Apply Fix 6 - KeyPool mark_rpm/mark_daily - Version 3 with raw unicode escapes"""
...
# Fix 6A: Daily quota handler - with actual escape sequences as they appear in file
old_daily = r'''                        if _is_daily:
                            print(f"       \u21b3 {_model}: daily quota exhausted \u2014 trying next", flush=True)
                            continue'''
new_daily = r'''                        if _is_daily:
...
if old_daily in code:
    ...
    print("FIX 6A SKIP - daily pattern not found")
...
if old_rpm in code:
    ...
    print("FIX 6B SKIP - rpm pattern not found")
